...
import React, { useState, useEffect } from 'react';
import {
  Typography, Button, Box, Grid, Card, Modal, TextField, Snackbar, Alert
} from '@mui/material';
...
function AboutUs({ isAuthenticated }) {
  ...
  const [modalOpen, setModalOpen] = useState(false);
  const [formData, setFormData] = useState({ username: '', email: '', message: '' });
  const [messageError, setMessageError] = useState(''); // State to handle error message
  const navigate = useNavigate();
        ...
        <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 400, bgcolor: 'background.paper', p: 4 }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <Button
              ...
              color="primary"
              onClick={handleSend}
              disabled={!!messageError} // Disable the button if there is an error
            >
              Send
            </Button>
          </Box>
